...
import { RegisterReceiptsService } from 'src/app/core/service/register-receipts.service';
import { realCitizenIdValidator } from 'src/app/shared/validators/real-citizen-id-validator';

@Component({
  selector: 'report-register',
  templateUrl: './report-register.component.html',
  styleUrls: ['./report-register.component.scss']
})
export class ReportRegisterComponent extends BaseForm implements OnInit {
  date = new Date()
  course = []
  fees = []
  amount : any = '0.00'
  ...
  constructor(public applicantSV:ApplicantService,
              ...
              public RegisterReceiptsService:RegisterReceiptsService,
  ...
  ngOnInit(): void {
    console.log('aaa')
    console.log(this.id)
    this.RegisterReceiptsService.getDate(this.id).subscribe(x=>{
      console.log(x)
      this.data = x
    })
